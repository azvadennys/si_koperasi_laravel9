...
@section('content')
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($akun as $key => $index)
                                            @php
                                                ...
                                                $jumlahpeminjaman = $index->jumlah + $index->jumlah * 0.02 * $index->lama_peminjaman;
                                                $sisapinjaman = $jumlahpeminjaman - $index->angsuran->sum('jumlah');
                                            @endphp
                                            <tr class="text-center">
                                                ...
                                                <td>
                                                    {{ 'Rp ' . number_format($jumlahpeminjaman, 0, ',', '.') }}
                                                </td>
                                                <td>
    ...
    @section('custom_html')
        @foreach ($akun as $index)
            <form action="{{ route('peminjaman.destroy', $index->id) }}" id="delete-form-{{ $index->id }}"
                method="post">
                @csrf
                @method('DELETE')
            </form>
